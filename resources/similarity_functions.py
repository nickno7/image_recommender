...
def get_similar_images(image_path, database_path, table_name, mode,
                        embeddings_file, number_pictures, several_inputs):
    if mode == "content":
    ...
    closest_vectors, closest_indices = calculate_scores(
        query_vector, embeddings, image_ids, number_pictures
    )
    ...
    closest_image_paths = [
        get_image_path(database_path, table_name, image_ids[i]) for i in closest_indices
    ]
    titles = [
        f"ID: {image_ids[i]}, Score: {closest_vectors[j]:.2f}"
        for j, i in enumerate(closest_indices)
    ]
